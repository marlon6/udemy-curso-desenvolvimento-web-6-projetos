var timeId = null; //variável que armazena a chamada da função Timout

function iniciaJogo(){
	var nivel_jogo = (window.location.search).replace("?", "");

	
	//alert("Nível: " + nivel_jogo);
...
function cria_baloes(qtde_baloes){
	...
	for ( var i = 1; i<= qtde_baloes; i++){
		...
		//appendChild adiciona novos elementos 
		//como filhos na div ao invés de substituir
	}
}

function estourar(e){
	var id_balao = e.id;
	document.getElementById(id_balao).setAttribute("onclick", ""); //retirar onclick de balão já estourado.
